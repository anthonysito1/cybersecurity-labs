...
Welcome! This repository serves as an index for my hands-on cybersecurity labs, write-ups, and exercises. My full articles, detailed methodologies, and analysis are chronicled over on my Medium Profile: (https://medium.com/@anthonysito1).

This is a collection of some of the highlights. Please feel free to look at my profile and see the 10's of other labs and LetsDefend cases I've done. 
...
*   **Key Skills Demonstrated:** 
    *   SIEM Log Analysis (Elasticsearch/Kibana)
    *   Windows Security Event ID Tracking (`4625` Authentication Failures, `4624` Success)
...
*   **Key Skills Demonstrated:**
    ...
    *   **Adversary Behavior Mapping:** Correlated a surge in `whoami` discovery commands with privilege escalation events.
*   **Defensive Takeaways:** Recommended immediate host isolation/quarantine to cut off active Command and Control (C2) communication channels.
...
*   **Objective:** Investigate an administrative initial access vector over Remote Desktop Protocol (RDP) and de-obfuscate a fileless PowerShell reverse shell payload executing in memory.
*   **Key Skills Demonstrated:**
    ...
    *   **Egress Traffic Validation:** Queried Sysmon Event ID `3` to cross-reference network connections, verifying the connection state to determine if active C2 interactive sessions or data exfiltration occurred.
...
*   **Key Skills Demonstrated:**
    *   **Linux Audit Logging:** Monitored administrative activity logs via `system.auth.ssh` and `system.auth.sudo` modules inside Kibana.
    *   **Anti-Forensics Triage:** Detected malicious execution of the `truncate` command targeting critical system audit files (`/var/log/auth.log` and `/var/log/syslog`) mapping to MITRE technique `T1070` (Indicator Removal).
    *   **SIEM Pipeline Validation:** Analyzed the technical behavior of the agent harvester pipeline, verifying that real-time log ingestion preserves forensic evidence despite local system wiping.
    *   **Post-Exploitation Timeline Auditing:** Audited shell command execution timelines to successfully verify attacker containment and scope of impact following a superuser session.
*   **Defensive Takeaways:** Advised immediate enforcement of key-based cryptographic SSH access, disabling native root SSH authorization flags (`PermitRootLogin no`), and implementing append-only system logging attributes.
...
*   **Key Skills Demonstrated:**
    *   **SIEM Log Correlation:** Utilized Splunk transaction-based queries to chronologically map network and host-based event artifacts back to a single malicious IP.
    ...
    *   **Incident Handling & Escalation:** Executed standard Tier 1 incident triage, identifying scope constraints, preparing remediation directives, and escalating to Tier 2 support.
...
### 🔹 Privilege Escalation: Unquoted Service Path & Runtime Compilation
*   **Lab Write-up:** [SOC Lab 24 — Unquoted Service Path Attack](YOUR_MEDIUM_LINK)
*   **Objective:** Emulate and investigate local privilege escalation via a Windows path interception vulnerability, tracking lateral movement vectors and runtime compilation behaviors via SIEM logging.
*   **Key Skills Demonstrated:**
    *   **Windows Internal Forensics:** Exploited and triaged Windows Service Control Manager parsing logic (`T1574.009`) by detecting unauthorized execution of root-level binaries (`C:\Program.exe`).
    *   **In-Memory Compilation Triage:** Parsed Sysmon Event ID `1` logs for high-fidelity privilege indicators, including PowerShell execution of `.NET` compilation arguments (`Add-Type`, `OutputAssembly`).
    ...
    *   **True Positive Verification:** Analyzed the deployment of anomalous host execution components to map compromised local credential usage (`backdoor` account access) back to the primary adversary source node.
...
There are several more case writeups on my medium as well, these are just what I believe are some of the best.
